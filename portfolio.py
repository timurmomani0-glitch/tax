    ...
    tab_overview = mo.vstack([
        mo.md(
            ...
            **Universe:** the "Magnificent 7" ({", ".join(MAG7)}) — large US firms
            ...
            Small in economic terms but statistically significant and robust across
            providers, sub-periods and winsorisation — see the **ESG & Valuation** tab.
            """
        ),
    ])
    return (tab_overview,)
...
def _(df_corr, df_desc, df_panel, df_reg, provider_note, px, sector_select):
    ...
    if df_reg is not None:
        ...
        _parts += [
            ...
            mo.md(
                "**Cross-tool validation (Week 10 self-exploration):** the identical "
                "models estimated in **R** (`analysis_R/WK10_ResultTables_AF1204.Rmd`, "
                "stargazer) are embedded below; the ESGscore coefficient agrees with "
                "the Python estimate to 4 decimal places."),
            mo.md("_R table: `data/regression_r.html` in the repo (generated by the "
                  "Rmd); its ESGscore row is the agreement check._"),
            mo.md(
        ...
        mo.md(
            ...
            **Layer A (offline pipelines, this repo, never deployed)** downloads and
            ...
            - **R + stargazer cross-validation (self-exploration)** — the same four
              models estimated in a second language; coefficients agree, which guards
              against silent specification errors in either implementation.
...
@app.cell
def _(tab_esg, tab_financial, tab_method, tab_overview, tab_risk):
    app_tabs = mo.ui.tabs({
